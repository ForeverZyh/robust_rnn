...
class AIGRUCell(Layer):
    ...
    def call(self, _inputs, _states, training=None):
        ...
        if self.implementation == 1:
            ...
            if self.reset_after:
                # recurrent_h = r.mul(matrix_inner[:, 2 * self.units:], self.recurrent_activation,
                #                     lambda x: x)
                recurrent_h = r * matrix_inner[:, 2 * self.units:]
            else:
                # recurrent_h = r.mul(h_tm1, self.recurrent_activation, lambda x: x)
                recurrent_h = r * h_tm1
        ...
        # p1 = z.mul(h_tm1, self.recurrent_activation, lambda x: x)
        # p2 = z.mul(hh, lambda x: (self.recurrent_activation(x) * (-1) + 1), self.activation, True)
        # h = p1 + p2
        h = z.GRU_merge1(self.recurrent_activation, h_tm1, hh, self.activation)
        print(h.c.shape, h.r.shape)
        # h = z * (h_tm1 - hh) + hh
        # h = z * h_tm1 + z * hh * (-1) + hh
